...
function PlatformCard(props: { name: string; href: string; note: string }) {
  return (
    <a
      href={props.href}
      target="_blank"
    ...
    >
      <p className="font-medium text-[#1B4332]">{props.name}</p>
      <p className="text-gray-400 text-xs mt-1">{props.note}</p>
    </a>
  )
}
    ...
    <section className="py-16 bg-[#FAF7F2] border-t border-gray-200">
      <div className="max-w-4xl mx-auto px-6 text-center">
        ...
        <p className="text-gray-500 text-sm mb-8 max-w-lg mx-auto">
          You will always get our best rate by booking directly, but we are also listed on these platforms if you would like to read more reviews first.
        </p>
        <div className="flex flex-wrap gap-4 justify-center">
          {platforms.map(function (p) {
            return <PlatformCard key={p.name} name={p.name} href={p.href} note={p.note} />
          })}
        </div>
      </div>
    </section>
