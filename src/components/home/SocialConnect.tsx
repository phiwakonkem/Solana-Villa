const socials = [
  {
    name: 'Instagram',
    handle: '@solana_villas',
    href: 'https://www.instagram.com/solana_villas/',
...
function SocialCard(props: { name: string; handle: string; href: string; bg: string; description: string }) {
  return (
    <a
      href={props.href}
      target="_blank"
    ...
    >
      <div className={'absolute inset-0 ' + props.bg + ' opacity-90 group-hover:opacity-100 transition-opacity'} />
      <div className="relative z-10">
        <p className="font-semibold text-lg">{props.name}</p>
        <p className="text-white/80 text-sm">{props.handle}</p>
        <p className="text-white/60 text-xs mt-1">{props.description}</p>
      </div>
    </a>
  )
}
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {socials.map(function (s) {
            return <SocialCard key={s.name} name={s.name} handle={s.handle} href={s.href} bg={s.bg} description={s.description} />
          })}
        </div>
      </div>
    </section>
