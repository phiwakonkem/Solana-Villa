    ...
    <div className="min-h-screen bg-[#FAF7F2]">
      <div className="bg-[#1B4332] text-white py-32 px-6 text-center">
        ...
        <p className="text-gray-300 max-w-xl mx-auto">
          Located within the Sabie River Eco Estate — private, peaceful, and minutes from Kruger.
        </p>
      </div>
        ...
        <div className="flex flex-col gap-20">
          {properties.map((property, i) => {
            if (property.status === 'coming-soon') {
              return (
                <div key={property.slug} className="grid md:grid-cols-2 gap-12 items-center">
                  <div className="relative rounded-2xl overflow-hidden h-96 bg-[#1B4332]/5 border-2 border-dashed border-[#1B4332]/20 flex items-center justify-center">
                    <div className="text-center px-8">
                      ...
                      <p className="text-[#1B4332] font-medium text-lg">Under Construction</p>
                      <p className="text-gray-500 text-sm mt-2">Follow us on Instagram for updates</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-[#D4A853] text-xs tracking-widest uppercase mb-2">{property.location}</p>
                    <h2 className="font-[Playfair_Display] text-4xl font-bold text-[#1B4332] mb-4">{property.name}</h2>
                    <p className="text-gray-500 leading-relaxed mb-6">
                      Our newest property is currently under construction in the Sabie River Eco Estate.
                      Follow our Instagram for build updates and be the first to know when bookings open.
                    </p>
            ...
            return (
              <div key={property.slug}
                ...
                <div className={`relative rounded-2xl overflow-hidden h-96 group ${i % 2 === 1 ? 'md:col-start-2' : ''}`}>
                  <img
                ...
                <div className={i % 2 === 1 ? 'md:col-start-1 md:row-start-1' : ''}>
                  <p className="text-[#D4A853] text-xs tracking-widest uppercase mb-2">{property.location}</p>
                  <h2 className="font-[Playfair_Display] text-4xl font-bold text-[#1B4332] mb-4">{property.name}</h2>
                  <p className="text-gray-600 leading-relaxed mb-6">{property.description.split('\n\n')[0]}</p>
                  ...
                  <Link href={`/villas/${property.slug}`}
                        className="inline-flex items-center gap-3 bg-[#1B4332] hover:bg-green-900 text-white px-8 py-4 rounded-xl transition-colors group/btn">
